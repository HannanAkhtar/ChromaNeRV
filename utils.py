...
def rgb_to_ycbcr_bt709(rgb):
    """Convert normalized gamma-coded RGB to normalized full-range BT.709 Y'CbCr."""
    if rgb.size(-3) != 3:
        raise ValueError(f'Expected 3 color channels, got shape {tuple(rgb.shape)}')
    red, green, blue = rgb.unbind(dim=-3)
    luma = BT709_KR * red + BT709_KG * green + BT709_KB * blue
    cb = 0.5 + (blue - luma) / (2 * (1 - BT709_KB))
    cr = 0.5 + (red - luma) / (2 * (1 - BT709_KR))
    return torch.stack((luma, cb, cr), dim=-3)


def ycbcr_to_rgb_bt709(ycbcr):
    """Convert normalized full-range BT.709 Y'CbCr to gamma-coded RGB."""
    if ycbcr.size(-3) != 3:
        raise ValueError(f'Expected 3 color channels, got shape {tuple(ycbcr.shape)}')
    luma, cb, cr = ycbcr.unbind(dim=-3)
    red = luma + 2 * (1 - BT709_KR) * (cr - 0.5)
    blue = luma + 2 * (1 - BT709_KB) * (cb - 0.5)
    green = (luma - BT709_KR * red - BT709_KB * blue) / BT709_KG
    return torch.stack((red, green, blue), dim=-3)


def model_output_to_rgb(output, color_space, clamp=False):
    rgb = output if color_space == 'rgb' else ycbcr_to_rgb_bt709(output)
    return rgb.clamp(0, 1) if clamp else rgb


def rgb_target_to_model_space(rgb, color_space):
    return rgb if color_space == 'rgb' else rgb_to_ycbcr_bt709(rgb)

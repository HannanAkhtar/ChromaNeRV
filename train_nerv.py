...
def main():
    ...
    parser.add_argument('--color_space', default='rgb', choices=['rgb', 'ycbcr'],
                        help='model output color space; ycbcr uses normalized full-range BT.709')
...
# NERV architecture parameters
    ...
    exp_id = f'{args.dataset}/{args.color_space}_embed{args.embed}_{args.stem_dim_num}_fc_{args.fc_hw_dim}__exp{args.expansion}_reduce{args.reduction}_low{args.lower_width}_blk{args.num_blocks}_cycle{args.cycles}' + \
...
def train(local_rank, args):
    ...
    if local_rank in [0, None]:
        ...
        if args.eval_only:
            ...
            tb_id = hashlib.sha1(args.exp_id.encode('utf-8')).hexdigest()[:12]
            writer = SummaryWriter(os.path.join('output', 'tensorboard', tb_id))
    else:
        writer = None
    ...
    if args.eval_only:
        ...
        print_str += f'RGB PSNR/MS-SSIM on validate set for bit {args.quant_bit} with axis {args.quant_axis}: {round(val_psnr[-1].item(),2)}/{round(val_msssim[-1].item(),4)}\n'
        print_str += f"PSNR-Y: {args.last_eval_metrics['psnr_y']:.2f}\n"
        print_str += f"Model params: {args.total_params:.6f}M\n"
        print_str += f"Decoding FPS: {args.last_eval_metrics['fps']:.2f}"
        print(print_str)
    ...
    for epoch in range(args.start_epoch, total_epochs):
        ...
        for i, (data,  norm_idx) in enumerate(train_dataloader):
            ...
            model_target = rgb_target_to_model_space(data, args.color_space)
            target_list = [F.adaptive_avg_pool2d(model_target, x.shape[-2:]) for x in output_list]
        ...
        if (epoch + 1) % args.eval_freq == 0 or epoch > total_epochs - 10:
            ...
            if local_rank in [0, None]:
                ...
                writer.add_scalar(f'Val/PSNR_Y_{h}X{w}_gap{args.test_gap}', args.last_eval_metrics['psnr_y'], epoch+1)
                writer.add_scalar(f'Val/MSSSIM_{h}X{w}_gap{args.test_gap}', val_msssim[-1], epoch+1)
                writer.add_scalar('Val/FPS', args.last_eval_metrics['fps'], epoch+1)
...
def evaluate(model, val_dataloader, pe, local_rank, args):
    ...
    for i, (data,  norm_idx) in enumerate(val_dataloader):
        ...
        rgb_output_list = [model_output_to_rgb(output, args.color_space, clamp=True) for output in output_list]

        # dump predictions
        if args.dump_images:
            for batch_ind in range(data.size(0)):
                full_ind = i * args.batchSize + batch_ind
                save_image(rgb_output_list[-1][batch_ind], f'{visual_dir}/pred_{full_ind}.png')
                save_image(data[batch_ind], f'{visual_dir}/gt_{full_ind}.png')

        # Compute comparable full RGB metrics and luma-only PSNR.
        target_list = [F.adaptive_avg_pool2d(data, x.shape[-2:]) for x in rgb_output_list]
        psnr_list.append(psnr_fn(rgb_output_list, target_list))
        msssim_list.append(msssim_fn(rgb_output_list, target_list))
        luma_output_list = [rgb_to_ycbcr_bt709(output)[:, :1] for output in rgb_output_list]
        luma_target_list = [rgb_to_ycbcr_bt709(target)[:, :1] for target in target_list]
        psnr_y_list.append(psnr_fn(luma_output_list, luma_target_list))
        val_psnr = torch.cat(psnr_list, dim=0)              #(batchsize, num_stage)
        val_psnr = torch.mean(val_psnr, dim=0)              #(num_stage)
        val_psnr_y = torch.cat(psnr_y_list, dim=0)
        val_psnr_y = torch.mean(val_psnr_y, dim=0)
        val_msssim = torch.cat(msssim_list, dim=0)          #(batchsize, num_stage)
        val_msssim = torch.mean(val_msssim.float(), dim=0)  #(num_stage)        
        if i % args.print_freq == 0:
            fps = fwd_num * sample_count / sum(time_list)
            print_str = 'Rank:{}, Step [{}/{}], RGB PSNR: {}, PSNR-Y: {}, RGB MSSSIM: {} FPS: {}'.format(
                local_rank, i+1, len(val_dataloader),
                RoundTensor(val_psnr, 2, False), RoundTensor(val_psnr_y, 2, False),
                RoundTensor(val_msssim, 4, False), round(fps, 2))
            print(print_str)
            if local_rank in [0, None]:
                with open('{}/rank0.txt'.format(args.outf), 'a') as f:
                    f.write(print_str + '\n')
    model.train()
    args.last_eval_metrics = {
        'rgb_psnr': val_psnr[-1].item(),
        'psnr_y': val_psnr_y[-1].item(),
        'rgb_msssim': val_msssim[-1].item(),
        'fps': fwd_num * sample_count / sum(time_list),
    }
